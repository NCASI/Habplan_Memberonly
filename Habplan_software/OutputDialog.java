...
public class OutputDialog extends JDialog {
  ...
  int savedSchedsCount=0; //keep track of n of saved schedules
    ReverseFileReader reverse; //read last line of old file to get save count
    boolean isReverseRead=false; //only read lastLine once
  //User written stuff
...
public void deleteAll(){
    ...
    /**
     *Reset savedSchedsCount if the saveSchedules file exists. This
     *ensures that it always properly increments.  This is needed
     *when someone restarts Habplan and then wants to save into existing
     *save files.
     */
    public int resetSavedSchedsCount(String filename){
	int num=savedSchedsCount;
        try{ reverse=new ReverseFileReader(filename);
	    String lastLine=reverse.readLine();
	    lastLine=reverse.readLine();
            num=lastLine.indexOf(",");
            lastLine=lastLine.substring(0,num);
            num=Integer.parseInt(lastLine);
            num++; //increment by 1
	}catch(Exception e){
           parent.console.println("Error reading last line of: "+filename);
...
*/
 ...
 if(isAppend && !isReverseRead)savedSchedsCount=resetSavedSchedsCount(fileName);
 isReverseRead=true; //read the last line, at most 1 time.
...
*/
 ...
 if(isAppend && !isReverseRead)savedSchedsCount=resetSavedSchedsCount(fName);
 isReverseRead=true; //read the last line, at most 1 time.
    ...
    for (int t=1;t<=yrs;t++){  //t is the scaled year, so that the first year is 1
	 ...
	 for (int j=1;j<gisData.polyID[i].length;j++){ //get total block size
      ...
      for (int j=1;j<gisData.polyID[i].length;j++){
       s=Integer.toString(savedSchedsCount)+sep;
